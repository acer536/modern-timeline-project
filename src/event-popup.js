...
function updateArrowVisibility() {
  ...
  nextBtn.classList.toggle(
    'is-hidden',
    popupCurrentIndex === popupImages.length - 1
  );
}
...
function handleTouchMove(e) {
  ...
  if (
    (popupCurrentIndex === 0 && diff > 0) ||
    (popupCurrentIndex === popupImages.length - 1 && diff < 0)
  ) {
...
export function showEventPopup(
  ...
  const hasImages =
    details.imageUrl &&
    Array.isArray(details.imageUrl) &&
  ...
  const fullDescription = details.fullDescription
    ? details.fullDescription[currentLang] || details.fullDescription['en']
    : '';
  const sourceText = details.sourceText
    ? details.sourceText[currentLang] || details.sourceText['en']
    : 'Learn More';
  ...
  // 綁定事件
  document
    .getElementById('eventPopupCloseButton')
  ...
  if (hasImages) {
    ...
    document
      .getElementById('popupNextButton')
      ?.addEventListener('click', showNextInPopup);
    document
      .getElementById('popupPrevButton')
      ?.addEventListener('click', showPrevInPopup);
